...
<div class="page-container">
		 <c:if test="${model=='审核'}">
 <div class="row cl" id="bankCheckStatusParentDiv" style="margin-top:20px;margin-left:20px;" > 	<label class="form-label col-xs-4 col-sm-2">银行审核状态
</label>
<div class="formControls col-xs-8 col-sm-9">
 <span class="select-box" style="width:200px;">
<select name="bankCheckStatus" id="bankCheckStatus" class="select">
 <c:forEach items="${bankCheckStatusList}" var="opt">
  <c:if test="${opt.id==data.bankCheckStatus}">
...
<c:if test="${model=='审核'}">
 <div class="row cl"  id="bankCheckRemarkParentDiv" style="margin-top:20px;margin-left:20px;" > 	<label class="form-label col-xs-4 col-sm-2">银行审核备注
</label>
<div class="formControls col-xs-8 col-sm-9">
<input type="text" class="input-text" value="${data.bankCheckRemark}" placeholder="请输入银行审核备注" id="bankCheckRemark" name="bankCheckRemark">
</div>
</div>
</c:if>
...
var bankCheckStatus = $("#bankCheckStatus").val();
param.bankCheckStatus=(bankCheckStatus==undefined?'':bankCheckStatus);
var bankCheckRemark = $("#bankCheckRemark").val();
param.bankCheckRemark=(bankCheckRemark==undefined?'':bankCheckRemark);
	...
	$.ajax({
	    ...
	    dataType : "json",
	    url: '${pageContext.request.contextPath}/bank/loadApplyInfo/save.do?'+hrefParam,
