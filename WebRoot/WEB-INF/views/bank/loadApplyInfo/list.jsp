 ...
 function search(){
   ...
   ajaxList();
	...
	function ajaxList() {
		...
		$.ajax({
					type : 'get',
					url : "${pageContext.request.contextPath}/bank/loadApplyInfo/toList.do",
					data : obj,
		...
		if(rows[i].schoolCheckStatus!='1' && rows[i].schoolCheckStatus!='3' && rows[i].bankCheckStatus!='2' && rows[i].bankCheckStatus!='3'){
	...
	function comfirmAction(e){
		...
		if(act=='add'||act=='update'||act=='detail'){
			var url = "${pageContext.request.contextPath}/bank/loadApplyInfo/"+act+".do?"+param;
