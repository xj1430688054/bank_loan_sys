...
<span style="width:200px;margin-left:50px;">所属学校</span>
 <select name="schoolId" id="schoolId" style="height:30px;width:250px;">
<option value="">全部</option>
<c:forEach items="${schoolIdList}" var="item2"> 
  <option value="${item2.id}">${item2.name}</option>
</c:forEach>
 </select>
<span style="width:200px;margin-left:50px;">是否黑名单</span>
 <select name="isBlack" id="isBlack" style="height:30px;width:250px;">
<option value="">全部</option>
<c:forEach items="${isBlackList}" var="item2"> 
  <option value="${item2.id}">${item2.name}</option>
</c:forEach>
 </select>
</span>
 ...
 function search(){
   ...
   ajaxList();
	...
	function ajaxList() {
		...
		$.ajax({
					type : 'get',
					url : "${pageContext.request.contextPath}/bank/studentInfo/toList.do",
					data : obj,
					...
					var  schoolId = rows[i].schoolIdStr;
					html += '<td>' + setNullToEmpty(schoolId) + '</td>';
					var  isBlack = rows[i].isBlackStr;
					html += '<td>' + setNullToEmpty(isBlack) + '</td>';
	...
	function comfirmAction(e){
		...
		if(act=='delete'||act=='updateByWithValue'){
			$.ajax({
				type : "post",
				url : "${pageContext.request.contextPath}/bank/studentInfo/"+act+".do",
				data : param,
				success : function(result) {
					alert(result.msg);
