...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
	 <span class="l" id="globalBtns">
	   				<a href="javascript:;" act="add" model="申请贷款"  isComfirm="0" comfirmTip="确认要申请贷款该数据吗？"  onclick="actionClick(this)" redirectUrl="bank_info"  class="btn btn-success radius">申请贷款</a>
 ...
 function search(){
   ...
   ajaxList();
	...
	function ajaxList() {
		...
		$.ajax({
					type : 'get',
					url : "${pageContext.request.contextPath}/student/loadApplyInfo/toList.do",
					data : obj,
		...
		if(rows[i].schoolCheckStatus!='2' && rows[i].schoolCheckStatus!='3' && rows[i].bankCheckStatus!='2' && rows[i].bankCheckStatus!='3'){
								html +='<a href="javascript:;" act="update" model="修改贷款信息"   onclick="actionClick(this)"   redirect-url="bankInfo" data-id="'+rows[i].id+'" is-comfirm="0" comfirm-tip="确认要修改贷款信息该数据吗？"  class="btn btn-primary radius">修改贷款信息</a>';

		}
		if(rows[i].schoolCheckStatus!='1' && rows[i].schoolCheckStatus!='3' && rows[i].bankCheckStatus!='1' && rows[i].bankCheckStatus!='3'){
								html +='<a href="javascript:;" act="redirect" model="我要还款"   onclick="actionClick(this)"   redirect-url="remoneyInfo" data-id="'+rows[i].id+'" is-comfirm="0" comfirm-tip="确认要我要还款该数据吗？"  class="btn btn-success radius">我要还款</a>';
	...
	function comfirmAction(e){
		...
		if(act=='add'||act=='update'||act=='detail'){
			var url = "${pageContext.request.contextPath}/student/loadApplyInfo/"+act+".do?"+param;
				window.location.href=url;
		 }
		if(act=='redirect'){
			var url = "${pageContext.request.contextPath}/student/loadApplyInfo/"+act+".do?"+param;
			creatIframe(url,'${tableZhName}'+model);
		 }
